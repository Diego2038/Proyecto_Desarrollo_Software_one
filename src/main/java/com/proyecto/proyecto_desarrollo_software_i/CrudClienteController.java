 ...
 * @author Diego
 */
public class CrudClienteController extends CrudSQL implements Initializable {
        ...
        if (ae.getSource() == btn_guardar_usuario) {
            ...
            try {
                if (verificadorSizeCelda(txt_cedula, 6, 10) && verificadorSizeCelda(txt_nombre, 7, 32) 
                    ...
                    crud_registrar("cliente", "cedula", "nombre", "telefono", "correo_electronico", 
                            txt_cedula.getText().trim(), txt_nombre.getText().trim(), txt_telefono.getText().trim(), 
                            txt_correo.getText().trim());
                    
                    borrarDatosTextField(componentesTextField);
                    info.setText("Cliente registrado exitosamente.");
            ...
            try {
                ...
                resultado = crud_buscar_manual(SELECCION, "cliente", CONDICION,NUM_SELECCION);
            ...
            try {
                //>>>>//
                crud_modificar_manual("cliente", ACTUALIZACION, CONDICION);
                info.setText("Cliente modificado exitosamente");
            } catch (SQLException ex) {
    ...
    public String seleccionarValor(String nombre){
        ...
        }else if("correo_electronico".equals(nombre)){
    ...
    public void cambiarVisibilidadRoundButton(ActionEvent ae){
        cb_condicion1.setValue("Cédula cliente");
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        String LISTA_SELECCION[] = {"-","Nombre cliente","Cédula cliente","Teléfono cliente",
                "Email cliente"}; 
        String LISTA_CONDICION[] = {"-","Nombre cliente","Cédula cliente","Teléfono cliente",
                "Email cliente"};
